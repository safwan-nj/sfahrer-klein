...
const Retour = () => {
  ...
  /* هذه هي الدالة الاصلية وكلا الدالتين اسفلها جديدتان */
  /* const handleArtikelNrInput = (e) => {
   ...
   /// هذه الدالة للإرسال عند الإدخال اليدوي
    const handleSubmitArtNr = () => {
      if (artNr.trim() !== "") {
        // المهمة الأولى: إرسال البيانات المدخلة يدوياً
        handleOpenPopup(artNr);

        // المهمة الثانية: إعادة التطبيق لوضع الماسح الضوئي
        // هذا السطر يخفي الكيبورد وزر التأكيد ويعيد السلوك الأصلي
        setShowKeyboard(false);
      }
    };

    // هذه الدالة تتعامل مع كل أنواع الإدخال
    const handleArtikelNrInput = (e) => {
        const value = e.target.value;
        setArtNr(value);
        setArtikelNrError("");

        // إذا كان الكيبورد غير ظاهر (وضع الماسح)، أرسل فوراً
        if (!showKeyboard) {
        if (value.trim() !== "") {
            handleOpenPopup(value);
        }
        }
        // إذا كان الكيبورد ظاهراً، انتظر التأكيد اليدوي
    };
      ...
      onConfirm: () => {
        const formattedData = {
          ...
          articles: entries.map((entry, index) => ({
            ...
            artikel_eancode: entry.artikelEancode,
            ...
            kleinQR: entry.kleinQR || "" // إضافة حقل Klein-QR
          }))
        };
        ...
        AxiosConnect('POST', "kis_rma_auftrag_save", '', formattedData)
          ...
          .catch((error) => {
            setCustomAlert({
              show: true,
              message: "Beim Speichern der Daten ist ein Fehler aufgetreten. Bitte versuchen Sie es erneut.",
              onConfirm: () => {
                setCustomAlert({ show: false, message: "", onConfirm: null, type: "default" });
                if (inputRef.current) {
                  inputRef.current.focus();
          ...
          console.log('Formatted Data toBeSaved:', formattedData); // Log the formatted data for debugging
        setCustomAlert({ show: false, message: "", onConfirm: null, type: "default" });
      },
      type: "default"
    });
  ...
  const handleOpenPopup = (inputValue = artNr) => {
    ...
    // التحقق من عدد العناصر قبل فتح النافذة المنبثقة
    if (entries.length >= 12) {
      setCustomAlert({
        show: true,
        message: "Sie haben das Maximum von 12 Artikeln erreicht. Bitte speichern Sie die aktuellen Artikel und starten Sie eine neue Rückgabe.",
        onConfirm: () => {
          setCustomAlert({ show: false, message: "", onConfirm: null, type: "default" });
          setArtNr("");
          if (inputRef.current) {
    ...
    AxiosConnect('POST', "kis_rma_scaninput", '', inputValue)
      .then((response) => {
        ...
        if (articleData) {
          if (articleData.response === "isArticle") {
            ...
            setFormData({
              ...formData,
              artikelEancode: articleData.artikel_eancode,
      ...
      .catch((error) => {
        setCustomAlert({
          show: true,
          message: 'Beim Abrufen der Daten ist ein Fehler aufgetreten. Bitte überprüfen Sie die Nummer und versuchen Sie es erneut.',
          onConfirm: () => {
            setArtNr("");
            setCustomAlert({ show: false, message: "", onConfirm: null, type: "default" });
            if (inputRef.current) {
        ...
        <div className={`sa-bg-main-light flex flex-col flex-grow overflow-y-hidden ${isKundePopupOpen || isPopupOpen ? 'pointer-events-none opacity-50' : ''}`}>
          ...
          <div className={`${kundeNummer ? 'sa-bg-main-light' : ''} sticky top-0 z-20 px-8 rounded-md pt-2`}>
            ...
            <div className="pr-3 pl-3 pt-1 pb-1 rounded-full sa-bg-main-dark-transparent -mx-6">
              <div className="flex items-center gap-2 p-1 relative">
                ...
                <InputPrimary
                    ...
                    onChange={handleArtikelNrInput} // <-- تستدعي الدالة الرئيسية
                ...
                <div className="absolute right-1 flex items-center">
                  <button
                      onClick={showKeyboard ? handleSubmitArtNr : handleShowKeyboard}
                      className={`p-2 rounded-full text-white ${showKeyboard ? 'bg-blue-500' : 'sa-bg-main-secondary'}`}
                      title={showKeyboard ? "Eingabe senden" : "Tastatur anzeigen"}
                  >
                      {showKeyboard ? <FaIcons.FaCheckDouble className="w-5 h-5" /> : <FaIcons.FaKeyboard className="w-5 h-5" />}
                  </button>
                </div>
              </div>
            ...
            <div className="items-center justify-between">
              <p className="p-1 mx-3 rounded-lg text-center text-xs font-semibold text-gray-400">
                Max 12 Artikel.
              </p>
            </div>
            )} */}
              ...
              <div className="mt-2 rounded-xl sa-bg-main-dark-transparent px-2 py-3 h-[59vh]" style={{ overflowY: 'scroll' }}>
                {entries.map((entry, index) => (
                  <ArtikelCard
                    ...
                    grund={entry.grund}
                    kleinQR={entry.kleinQR} // إضافة kleinQR
                  />
                ))}
              </div>
            </div>
